...
export default function HomeScreen() {
  ...
  // Thick radiating glow — same on all themes (like reference image)
  const glowRgb = isCmd ? cmdRedRgb : a;
  const radiatingGlow = '0 0 4px rgba(' + glowRgb + ',0.8), 0 0 10px rgba(' + glowRgb + ',0.6), 0 0 25px rgba(' + glowRgb + ',0.4), 0 0 50px rgba(' + glowRgb + ',0.2), 0 0 80px rgba(' + glowRgb + ',0.1), 0 0 120px rgba(' + glowRgb + ',0.05)';
    ...
    <SafeAreaView style={[styles.container, Platform.OS === 'web' && { backgroundImage: isNeon ? 'linear-gradient(135deg, rgba(' + a + ', 0.7) 0%, rgba(' + a + ', 0.3) 25%, rgba(0,0,0,0.85) 55%, #000 100%)' : isLiquid ? 'linear-gradient(160deg, #1a1a1e 0%, #111113 40%, #0a0a0c 100%)' : isCmd ? 'none' : 'none' }]}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={onRefresh} tintColor={ac} colors={[ac]} />}>
        ...
        <View style={styles.connectedBotsSection}>
          ...
          {primaryEA && (
            <View style={[styles.neonWrap, !isNeon && { padding: 0 }, Platform.OS === 'web' && { boxShadow: radiatingGlow, borderRadius: shapeR + 8, width: shapeWidth } as any]}>
          ...
          {primaryEA && (
            <View style={[styles.neonWrapInfo, !isNeon && { padding: 0 }, Platform.OS === 'web' && { boxShadow: radiatingGlow, borderRadius: shapeRInfo + 8, width: shapeWidth } as any]}>
...
{/* ========== 4. ADD EA — LIQUID GLASS PILL ========== */}
          <View style={[styles.neonWrapPill, !isNeon && { padding: 0 }, Platform.OS === 'web' && { boxShadow: radiatingGlow, borderRadius: shapeRAdd + 8, width: shapeWidth } as any]}>
